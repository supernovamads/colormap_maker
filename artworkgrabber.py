...
print('NOW GETTING THE ARTWORK')
# artwork_response = urllib.request.urlretrieve(artwork_url,'../artwork/'+artist.replace(' ','').lower()+
#                                               '_'+album.replace(' ','').lower()+'.jpg',timeout=5)

artwork_request = urllib.request.urlopen(artwork_url, timeout=5).read()
with open('artwork/'+artist.replace(' ','').lower()+'_'+album.replace(' ','').lower()+'.jpg', 'wb') as f:
    try:
        f.write(artwork_request.content)
    except:
        print('error')